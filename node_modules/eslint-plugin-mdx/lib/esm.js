    ...
    traverse(ast, {
        code: function (node, parent) {
            ...
            if (node.lang) {
                ...
                blocks.push(__assign(__assign({}, node), { baseIndentText: getIndentText(text, node), comments: comments, rangeMap: getBlockRangeMap(text, node, comments) }));
            }
        },
    });
    return blocks.map(function (block, index) { return ({
        filename: index + "." + block.lang,
        text: __spreadArray(__spreadArray([], block.comments), [block.value, '']).join('\n'),
    }); });
}
/**
...
function adjustBlock(block) {
    ...
    return function adjustMessage(message) {
        ...
        return __assign(__assign(__assign({}, message), out), adjustedFix);
    };
}
/**
...
var remark = {
    ...
    preprocess: function (text, filename) {
        ...
        return __spreadArray(__spreadArray([], markdown.preprocess(text, filename).map(function (_a) {
...
export { DEFAULT_LANGUAGE_MAPPER, base, codeBlocks, configs, getGlobals, getRemarkProcessor, getShortLang, noJsxHtmlComments, noUnusedExpressions, overrides$1 as overrides, processorOptions, processors, recommended, remark$1 as remark, remarkProcessor, requirePkg, rules };
